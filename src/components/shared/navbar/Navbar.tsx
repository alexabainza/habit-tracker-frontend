  ...
  return (
    currentUser ? (
      <>
        ...
        {isBurgerOpen && <div onClick={toggleBurgerMenu} className="bg-black/40 h-dvh w-dvw absolute top-0 left-0 transition-opacity duration-300" />}
        <nav className={`border-r border-opacity-30 border-gray-700 absolute md:flex md:relative md:translate-x-0 top-0 min-h-dvh max-w-16 flex flex-col items-center justify-between text-[var(--color-primary)] bg-white p-5 shadow-sm dark:bg-gray-950 transition-all duration-300 z-50 ${isBurgerOpen ? "translate-x-0" : "-translate-x-20"}`}>
          <Link
            to='/'
